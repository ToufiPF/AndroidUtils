...
object NumberPickerTestUtils {
    ...
    private val mDecrement = ViewActions.actionWithAssertions(
        GeneralClickAction(
            Tap.SINGLE,
    ...
    private fun checkPreconditions(view: View?, value: Int, description: String?): NumberPicker {
        val picker = view as? NumberPicker ?: throw PerformException.Builder()
            .withActionDescription(description).withViewDescription(HumanReadables.describe(view))
            .withCause(ClassCastException("view is not a NumberPicker")).build()

        if (value !in picker.minValue..picker.maxValue) throw PerformException.Builder()
            .withActionDescription(description).withViewDescription(HumanReadables.describe(view))
            .withCause(IllegalArgumentException("Value $value is not in the [${picker.minValue}, ${picker.maxValue}] range of the NumberPicker"))
            .build()
    ...
    fun setValueByJumping(value: Int): ViewAction = object : ViewAction {
        ...
        override fun perform(uiController: UiController?, view: View?) {
            ...
            when {
                !(picker.minValue <= value && value <= picker.maxValue) -> throw IllegalArgumentException(
                    "Invalid value (not ${picker.minValue} <= $value <= ${picker.maxValue})"
                )
